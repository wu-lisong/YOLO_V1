#encoding:utf-8
'''
This file defines the dataset for yolo detector
@parameters:
    root            the fold path which contains the real images to be trained
    list_file       image_name x1,y1,x2,y2,C
    train
    transform
'''
import os
...
class yoloDataset(torch.utils.data.Dataset):
    ...
    def __init__(self,root,list_file,train,transform):
        ...
        self.fnames = []     #contains the names of images to be processed
        self.boxes = []      #contains all the information about the location of the objects
        self.labels = []     #contains all the information about the classes of the objects 
    ...
    def __getitem__(self,idx):
        ...
        image = self.BGR2RGB(image) #because pytorch pretrained model use RGB
        image = self.subMean(image,self.mean) #减去均值
        image = cv2.resize(image,(self.image_size,self.image_size))
        target = self.encoder(boxes,labels)# SxSx30
